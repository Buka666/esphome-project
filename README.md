...
4. Рекурсивно собирает firmware-бинарники (`firmware*.bin`) по workspace в `dist/` (с исключением `dist/` и `.git/`), сохраняя короткие имена (`<env>-firmware.bin`), и формирует JSON-манифесты для OTA.
