...
Workflow запускается на push только в `main`/`master` (без отдельного запуска на auto-tag), а для Pull Request — только на события `opened/synchronize/reopened` и при изменениях в `devices/**`, `packages/**` и `.github/workflows/build.yml`.
